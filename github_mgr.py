...
-------------------------
Token-authenticated GitHub management script to:
- Check authenticated user details
- List repositories
- Create a new repository on GitHub
- Push local repository / code to GitHub
- Create repo AND push local code in one step
...
DEFAULT_OWNER = "ytbro8326-sudo"

def get_headers(token=None):
    tok = token or os.environ.get("GITHUB_TOKEN", DEFAULT_TOKEN)
    return {
        "Authorization": f"token {tok}",
        "Accept": "application/vnd.github.v3+json",
        "User-Agent": "GitHub-Mgr-Python"
    }
...
def list_repos(token=None):
    ...
    repos = api_request(url, "GET", token=token)
    return repos
